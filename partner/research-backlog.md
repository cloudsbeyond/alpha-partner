...
This backlog organizes deep research for the partner workspace. The goal is not a generic Agent report; the goal is to improve human-agent peer collaboration and agent-native project work.

## Research Rules

- Prefer primary sources: official docs, research papers, engineering blogs, product docs.
- Extract mechanisms and failure modes.
- Map each source back to this workspace.
- Do not treat Copilot/GitHub issue-to-PR agents as the main model; they are execution-layer references.
- Record useful conclusions in `evidence-index.md` or `decision-log.md`.
...
- Compare local workspace primitives against frontier harness primitives: tools, skills, `AGENTS.md`, shell, patch, sandbox, manifest, durable execution.
...
- How should context and memory be shaped so the agent remains useful without polluting the workspace or global memory?
...
- Local projects in `project-map.md`.
...
- How should a端侧 agent learn usage patterns, infer useful interruption timing, and recover user data permission from centralized super apps without becoming another opaque observer?

Primary anchors:

- Local `project-map.md` sample: 端侧 Agent 数据权限回收.
- User's 云起 / CloudsBeyond thinking when a concrete source is provided or inspected in a project session.
