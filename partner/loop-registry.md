...
Trigger: `alphaX daily radar`. Purpose: refresh `partner/focus-radar.md`; identify best next work block; expose stale branch, uncommitted state, false completion, wrong-layer work. Inputs: focus-radar, session-ledger, context-snapshot output for active projects, project-local AGENTS.md/README/specs/contracts. Output: compact focus/risk report. Default: read files, run read-only status commands, recommend one next focus move. Requires approval: writing target project files, scheduling, publishing/messaging reports.

## Loop 2: Source Drift Watch

Trigger: `alphaX source drift check`. Purpose: detect disagreement between spec, contract, implementation, tests, evidence. Candidate surfaces: agent-interaction-bridge architecture contracts/AI Contract Index; online_community Session Runtime spec/schema/tests/guardrails. Output: drift report with evidence and one recommended repair path. Default: run project-local contract checks and test commands when safe; do not edit files unless user asks for repair.

## Loop 3: False Completion Watch

Trigger: `alphaX false completion check`. Purpose: catch cases where tests pass but product acceptance, manual review, or human confirmation is incomplete. Candidate surfaces: online_community manual acceptance; agent-interaction-bridge partial contract gaps; generated artifacts with harness evidence but missing human acceptance. Output: list of claims proven, not proven, or blocked by missing acceptance. Default: compare test results against acceptance criteria; mark missing evidence explicitly.
