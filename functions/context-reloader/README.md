...
It does not store third-party project state in `alpha-partner`. It reads the
target project's live source and ignored `.alphaX/` objective data, then
produces a report-first re-entry output. The resulting alphaX judgments belong
to alphaX process data assets, not to the target project's objective data.
...
-> identify target project and project key
...
-> write back only with explicit user approval, and only into the target project's own language
```

Default boundary: **no auto write-back**.

Write-back to a third-party project requires explicit user confirmation and must
...
Before using it for a project decision, alphaX must read the live external
