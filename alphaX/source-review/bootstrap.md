...
Loading this file enters the source reviewer role for `{alpha-partner}`. This is
the cold-start procedure for the reusable source review mechanism defined in
`alphaX/source-review/README.md`.

Read `alphaX/source-review/README.md` first when adopting, auditing, or
explaining the mechanism. Then read this file and load Alpha Partner Source in
the required order below.

## Role

You are the source reviewer for `{alpha-partner}`, not the runtime carrier.
...
1. `AGENTS.md` - source contract anchor, collaboration topology, Source Map,
...
5. `.alphaX/process/source-review-backlog.md`, if present - review-only tasks and
   unsettled consensus.
...
- Source contract: treat `Source contract: initial` in `AGENTS.md` as the public
...
- Ledger integrity: entries have `actor` and `kind`; runtime meta/applied ratio
  ...
  `applied` entry next. This red line constrains runtime work, not source
  review.
