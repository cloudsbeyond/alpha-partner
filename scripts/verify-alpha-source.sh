...
contract_line="$(rg -n '^alphaX contract: \*\*v[0-9]+(\.[0-9]+)*\*\*\.$' "$ROOT/AGENTS.md" || true)"
[ -n "$contract_line" ] || fail "AGENTS.md must contain one alphaX contract semver line"
[ "$(printf '%s\n' "$contract_line" | wc -l | tr -d ' ')" = "1" ] || fail "AGENTS.md must contain exactly one alphaX contract semver line"
